...
spotless {
  ...
  kotlinGradle {
    ...
    // Google style, whose 2-space continuation indent matches the rest of the project.
...
//
// This is a method of an object rather than a top-level function of the build script,
// because a task action that calls a top-level function captures the script instance,
...
object JavadocFonts {
  ...
  fun removeDejaVuFontImport(javadocDir: File?) {
    ...
    javadocDir.walkTopDown().forEach { file ->
      if (file.isFile && (file.name.endsWith(".css") || file.name.endsWith(".html"))) {
...
val requireJavadoc =
  tasks.register<JavaExec>("requireJavadoc") {
    ...
    // The marker is a local variable rather than a script-level one, because a task action that
    // reads a script-level variable captures the script object, which the configuration cache
    // cannot serialize.
...
tasks.register<Exec>("tags") {
  ...
  argumentProviders.add(
    CommandLineArgumentProvider {
      sourceFiles.files.sorted().map {
        projectDirPath.relativize(it.toPath()).toString()
      }
    }
  )
}
